                ...
                if any(
                    ...
                    for sub_key in special_keys:  # for each special in cfg
                        if sub_key_cfg := key_cfg.get(sub_key, None):  # "_datasets_"
                            # key_cfg_excl_sub_key = OmegaConf.masked_copy(key_cfg, sub_key)
                            for name in sub_key_cfg:  # source: ... target: ...
                                # merge True by default
                                # cfg[key][sub_key][name] = deepcopy(key_cfg_excl_sub_key)
    ...
    Example:
        ...
        .. code-block:: yaml
            ...
            dataset_cfg:
                ...
                _apply_:
                    my_transform:
                        _target_: src.custom.utils.my_transform
                        _partial_: True
...
def config_callbacks(cfg: DictConfig, cb_cfg: DictConfig) -> DictConfig:
    ...
    for key in cb_cfg:
        if key == "_self_":
            cfg = hydra.utils.call(cb_cfg.get(key), cfg)
        else:
